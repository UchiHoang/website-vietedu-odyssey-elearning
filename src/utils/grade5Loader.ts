...
export const loadGrade5Curriculum = async (): Promise<any> => {
  console.log("Attempting to load curriculum from Supabase...");
  
  const { data, error } = await supabase
    .from("curriculumGrade5" as any)
    .select("*")
    .limit(1);  
  ...
  if (!data || data.length === 0) {
    console.error("No curriculum data found in table 'curriculumGrade5'");
    
    // Let's check if table exists by trying a different query
    const { error: tableCheckError } = await supabase
      .from("curriculumGrade5" as any)
      .select("count", { count: 'exact', head: true });
    
    if (tableCheckError) {
      console.error("Table might not exist:", tableCheckError);
      throw new Error(`Table 'curriculumGrade5' might not exist: ${tableCheckError.message}`);
    }
    
    throw new Error("No curriculum data found - table is empty");
  }
  
  console.log("Successfully loaded curriculum:", data[0]);
